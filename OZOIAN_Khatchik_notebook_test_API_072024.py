...
if uploaded_file is not None:
    ...
    if 'TARGET' not in data.columns:
        st.write("Erreur : La colonne 'TARGET' n'existe pas dans les données. Assurez-vous que les données contiennent cette colonne pour visualiser les distributions par classe.")
    else:
        ...
        if st.session_state['predictions'] is not None:
            ...
            if not predire_tous:
                ...
                if score is not None:
                    ...
                    st.write("Erreur : Le score n'a pas été trouvé dans les prédictions.")
                    
        # Tableau de bord d'informations sur le client
        if not predire_tous and 'score' in locals():
